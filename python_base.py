...
# a = 1024;
# print(a.bit_length())    # 11    10000000000
...
# decimal.getcontext().prec = 2   # 设置全局精度为4 即小数点后边4位
...
# print(a)    # {3, 'e', 5, 'o', 'H', 'l', 9, 10}
# b = t & s;    t.intersection(s)              # t 和 s的交集
# print(b)    # set()：因为没有交集所以是空的  {'H'}：在s中加入"H"就会显示这个
# c = t - s;    t.difference(s)                # 求差集（项在t中, 不在s中）
# print(c)    # {'H', 'e', 'o', 'l'}
# d = t ^ s;    t.symmetric_difference(s)      # 对称差集（项在t或s中, 但不会同时出现在二者中）
# print(d)    # {3, 'o', 5, 'l', 9, 10, 'e', 'H'}
...
# a = s.copy();
# print(a)    # {3, 42, 37, 5, 9, 10}
# a.discard(x);                                # 删除s中x
# print(a)    # {42, 37, 5, 9, 10}
# a.clear()                                    # 清空s
# print(a)    # set()
# {x**2 for x in [1, 2, 3, 4]}                 # 集合解析，结果：{16, 1, 4, 9}
# print({x**2 for x in [1, 2, 3, 4]}) # {16, 1, 4, 9}
# {x for x in 'spam'}                          # 集合解析，结果：{'a', 'p', 's', 'm'}
# print({x for x in 'spam'})  # {'s', 'm', 'a', 'p'}
#
# #-- 集合frozenset，不可变对象
# """
# set是可变对象，即不存在hash值，不能作为字典的键值。同样的还有list等(tuple是可以作为字典key的),还不能用作另一个集合的元素
...
# print(S)    # \temp
# S = b'Spam'                             # Python3中的字节字符串，使用索引取出来以ASCII码展示，S[0]：83
# print(S)    # b'Spam'
...
# print('a {1} {0} parrot'.format('kind', 'red'))     # a red kind parrot
# for x in S:                    # 字符串迭代，成员关系
#     print(x)    #s
#                 #p
#                 #a
#                 #m
# # [x*2 for x in s]                        # 字符串列表解析
# print([x*2 for x in S])     # ['ss', 'pp', 'aa', 'mm']
# # ','.join(['a', 'b', 'c'])               # 字符串输出，结果：a,b,c
# print(','.join(['a', 'b', 'c']))    # a,b,c
# print(repr(','.join(['a', 'b', 'c'])))    # 'a,b,c'
#
# #-- 内置str处理函数：
str1 = "stringobject"
...
str1="string object"
print(str1.title())     # String Object

width=20
str1.ljust(width)                       # 获取固定长度，左对齐，右边不够用空格补齐
print(str1.ljust(width)+'a')   # string object       a
str1.rjust(width)                       # 获取固定长度，右对齐，左边不够用空格补齐
print(str1.rjust(width))   #        string object
str1.center(width)                      # 获取固定长度，中间对齐，两边不够用空格补齐
print(str1.center(width))   #    string object    
